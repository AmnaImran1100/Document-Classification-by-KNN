...
# Read the data
try:
    data = pd.read_csv('Processed.csv', encoding='latin1')
except FileNotFoundError:
    print("Error: 'merged_file.csv' not found. Please check the file path.")

# Create graph representations for test documents
test_documents = [
    create_graph(str(data.iloc[42]['Content'])),
    create_graph(str(data.iloc[43]['Content'])),
    create_graph(str(data.iloc[44]['Content'])),
    create_graph(str(data.iloc[27]['Content'])),
    create_graph(str(data.iloc[28]['Content'])),
    create_graph(str(data.iloc[29]['Content'])),
    create_graph(str(data.iloc[12]['Content'])),
    create_graph(str(data.iloc[13]['Content'])),
    create_graph(str(data.iloc[14]['Content'])),

]

# Assuming you have actual categories for test documents
actual_categories = ['Travel', 'Travel', 'Travel', 'Finance', 'Finance', 'Finance', 'Food', 'Food', 'Food']  # Update with actual category labels
